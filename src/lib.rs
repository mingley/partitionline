...
//! [`protocol::api::MetadataResponse::brokers_by_id`] /
//! [`protocol::api::MetadataResponse::controller`] are Java
...
//! `controller` is null when `controllerId` is missing from Brokers).
...
//! [`protocol::api::MetadataRequest::for_topic_names_range`] /
//! [`protocol::api::MetadataRequest::build`] /
...
//! `Builder(List names, boolean allowAuto, short minVersion, short maxVersion)` /
//! `Builder.build` /
...
//! oldest and latest to `allowedVersion`; the four-argument Builder takes
//! `minVersion` and `maxVersion` independently; `Builder.build` rejects
//! versions older than 1, `allowAutoTopicCreation` false below v4, and a
//! null Name or non-zero TopicId below v12; topic IDs are empty when all
...
//! `from_groups_partition_data`; writes the `throttleTimeMs` argument;
//! convenience encode still writes throttle `0`; Java constructor is
//! v8+);
...
//! writes it; [`protocol::share::encode_share_fetch_response`] still
//! writes null;
...
//! [`protocol::share::ShareFetchResponse::of`] is Java
//! `ShareFetchResponse.of` (Responses from `toMessage`; ThrottleTimeMs /
//! ErrorCode / NodeEndpoints from the arguments; AcquisitionLockTimeoutMs
//! stays `0`);
...
//! ThrottleTimeMs JSON `0+`; convenience fills `0`);
//! [`protocol::admin::ListConfigResourcesRequest::supported_resource_types`] is Java
//! `ListConfigResourcesRequest.supportedResourceTypes` (v0 `CLIENT_METRICS`
...
//! [`protocol::admin::DescribeDelegationTokenResponse::error_counts`] is Java
//! `DescribeDelegationTokenResponse.errorCounts` (top-level `errorCode` only,
//! including `NONE`; Java `Collections.singletonMap`);
...
//! request ProducerEpoch is JSON `0+` (decode returns it last; encode already
//! takes `producer_epoch`; official Java `AddOffsetsToTxnRequestData.producerEpoch`;
//! not EndTxn response ProducerEpoch / InitProducerId / TxnOffsetCommit ProducerEpoch /
...
//! AddPartitionsToTxn ProducerId / WriteTxnMarkers ProducerId / ProducerEpoch);
//! request ProducerEpoch is JSON `0+` (decode returns it last; encode already
//! takes `producer_epoch`; official Java `TxnOffsetCommitRequestData.producerEpoch`;
//! not AddOffsetsToTxn ProducerEpoch / EndTxn response ProducerEpoch / InitProducerId /
//! AddPartitionsToTxn ProducerEpoch / WriteTxnMarkers ProducerEpoch / ProducerId);
...
//! [`protocol::api::ProducePartitionResponse::partition_response_with_message`] is Java
//! `ProduceResponse.PartitionResponse(Errors, String)` (same offset
//! sentinels as `PartitionResponse(Errors)`; empty `recordErrors`; Java
//! `lastOffset` is not stored).
//! [`protocol::api::ProducePartitionResponse::partition_response_with_record_errors`] is Java
...
//! is not stored).
//! [`protocol::api::ProducePartitionResponse::partition_response_with_current_leader`] is Java
//! `ProduceResponse.PartitionResponse(..., LeaderIdAndEpoch)` (`recordErrors` /
//! `errorMessage` / CurrentLeader from the arguments; Java `lastOffset` is
//! not stored).
...
//! plus each partition-level code, including `NONE`);
//! [`protocol::admin::AlterShareGroupOffsetsRequest::error_response`] is Java
//! `AlterShareGroupOffsetsRequest.getErrorResponse` (empty Responses;
//! ErrorMessage JSON-null; ThrottleTimeMs JSON `0+`; convenience fills `0`);
