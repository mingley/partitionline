...
/// ListOffsets v1 (classic). Isolation is v2+.
...
pub fn decode_list_offsets_request<B: Buf>(
    ...
    Ok((isolation, topic, partition, timestamp))
}
...
mod tests {
    ...
    fn list_offsets_v2_roundtrip() {
        ...
        encode_list_offsets_request(&mut req, 2, 1, "t", 3, EARLIEST_TIMESTAMP).unwrap();
        let (iso, topic, part, ts) = decode_list_offsets_request(&mut &req[..], 2).unwrap();
        assert_eq!((iso, topic.as_str(), part, ts), (1, "t", 3, -2));
        let mut resp = BytesMut::new();
        encode_list_offsets_response(&mut resp, 2, "t", 3, 0, -1, 7).unwrap();
        let (err, rts, off) = decode_list_offsets_response(&mut &resp[..], 2).unwrap();
