...
impl OffsetFetchResponse {
    ...
    /// the Java `throttleTimeMs` argument.
    ...
    /// Java `OffsetFetchResponse(int, Map, Map)` encode.
    ///
    /// Groups are [`Self::from_groups_partition_data`]. ThrottleTimeMs
    /// is written from `throttle_time_ms` (JSON `3+`; always on the wire
    /// for this helper). Convenience
    /// [`encode_offset_fetch_groups_response`] still writes throttle
    /// `0`. Java's constructor is v8+; v0–v7 Topics is
    /// [`Self::encode_from_partition_data`]. This crate speaks 8–9 for
    /// this helper. This is not [`Self::from_groups_partition_data`] /
    /// [`Self::from_groups`] / Request getErrorResponse.
...
impl OffsetFetchRequest {
    ...
    /// speaks 1–9. This is not [`Self::from_partitions`] / [`Self::for_groups`]
    /// / [`Self::build`] / [`Self::groups`] / [`OffsetFetchGroup::new`] /
    /// getErrorResponse.
    #[must_use]
    ...
    /// Java `OffsetFetchRequest.Builder(Map groupIdToTopicPartitionMap, boolean requireStable, boolean throwOnFetchStableOffsetsUnsupported)`.
    ///
    /// One group per map entry, in caller order (Java `entrySet`;
    /// `HashMap` order is unspecified). Topics from
    /// [`Self::from_partitions`]. `None` partitions is all committed
    /// partitions. MemberId is null; MemberEpoch is `-1` (JSON defaults;
    /// this Builder does not set them). RequireStable is the argument;
    /// [`Self::build`] still applies the version check.
    /// `throwOnFetchStableOffsetsUnsupported` is not part of this helper.
    /// Duplicate group ids keep both (this crate takes an iterator; Java
    /// `Map` keys are unique). Encode still writes independently. This
    /// crate speaks 1–9 (batched Groups need v8+). This is not
    /// [`Self::for_group`] / [`Self::from_partitions`] / [`Self::build`] /
    /// [`Self::groups`] / getErrorResponse.
    #[must_use]
...
impl OffsetCommitResponse {
    ...
    /// Java `OffsetCommitResponse(int, Map)` Topics.
    ///
    ...
    /// the JSON default (`0`) on v3+ (omitted on v2).
    ...
    /// Java `OffsetCommitResponse(int, Map)` encode.
    ///
    /// Topics are [`Self::from_errors`]. ThrottleTimeMs is written on
    /// v3+ from `throttle_time_ms`. Below v3 the field is omitted even
    /// when that value is non-zero. Decode fills `0`. Convenience
    /// [`encode_offset_commit_topics_response`] still writes throttle
    /// `0`. This crate speaks 2–9. This is not [`Self::from_errors`] /
    /// [`Self::add_partition`] / [`Self::merge`] / getErrorResponse.
    ...
    /// Java `OffsetCommitResponse.Builder.addPartition`.
    ///
    ...
    /// writes independently. This crate speaks 2–9. This is not
    ...
    /// Java `OffsetCommitResponse.Builder.addPartitions`.
    ///
    /// Finds or creates the topic by name (first-seen order), then
    /// appends each partition with a shared error code (`ArrayList`;
    /// duplicates are kept). An empty partition list still creates the
    /// topic (Java `getOrCreateTopic` before `forEach`). `partitions`
    /// is Java `List` after the partition-index function. Encode still
    /// writes independently. This crate speaks 2–9. This is not
    /// [`Self::add_partition`] / [`Self::from_errors`] / [`Self::merge`].
    #[must_use]
...
/// v8. GenerationIdOrMemberEpoch is JSON `1+` (INT32 after GroupId;
/// decode returns it last; JSON default `-1`). This crate speaks 2–9.
/// v0–v1 and v10+ are not spoken.
#[expect(
    clippy::too_many_arguments,
...
/// group_instance_id, generation_id)`.
///
/// GenerationIdOrMemberEpoch is JSON `1+` (always on the wire for spoken
/// versions; last). Decode below v6 fills
/// [`RecordBatch::NO_PARTITION_LEADER_EPOCH`] for omitted
/// `CommittedLeaderEpoch`. RetentionTimeMs is omitted outside v2–v4;
/// decode fills [`DEFAULT_RETENTION_TIME`]. Below v7 GroupInstanceId is
/// omitted; decode fills `None`.
#[expect(
    clippy::type_complexity,
...
mod tests {
    ...
    fn offset_commit_response_encode_from_errors_matches_java() {
        // Java 4.0 OffsetCommitResponse(int, Map): Topics from from_errors;
        // ThrottleTimeMs from the argument on v3+. Official Java
        // OffsetCommitResponse(int, Map). Convenience encode still writes
        // ThrottleTimeMs 0. This crate speaks 2-9. This is not from_errors
        // leftover / addPartition leftover / merge leftover /
        // getErrorResponse leftover.
        let rows = [
            ("a", 0, crate::error::UNKNOWN_TOPIC_OR_PARTITION),
            ("b", 0, crate::error::NOT_LEADER_OR_FOLLOWER),
            ("a", 1, 0i16),
    ...
    fn offset_commit_response_add_partition_matches_java() {
        // Java 4.0 OffsetCommitResponse.Builder.addPartition: getOrCreate
        ...
        // OffsetCommitResponse.Builder.addPartition. Encode still writes
        // independently. This crate speaks 2-9. This is not from_errors
        // leftover / merge / getErrorResponse.
        assert_eq!(
    ...
    fn offset_commit_response_add_partitions_matches_java() {
        // Java 4.0 OffsetCommitResponse.Builder.addPartitions: getOrCreate
        // topic by name, then partitions().add for each element with a
        // shared Errors. Empty Topics plus an empty list still creates
        // the topic (getOrCreateTopic before forEach). A non-empty list
        // equals repeated addPartition. Official Java
        // OffsetCommitResponse.Builder.addPartitions. Encode still
        // writes independently. This crate speaks 2-9. This is not
        // addPartition leftover / from_errors leftover / merge /
        // getErrorResponse.
    ...
    fn offset_fetch_response_encode_from_groups_partition_data_matches_java() {
        // Java 4.0 OffsetFetchResponse(int, Map, Map): Groups from
        // from_groups_partition_data; ThrottleTimeMs from the argument.
        // Official Java OffsetFetchResponse(int, Map, Map). Convenience
        // encode still writes ThrottleTimeMs 0. Java constructor is v8+.
        // This crate speaks 8-9 for this helper. This is not
        // from_groups_partition_data leftover / from_groups leftover /
    ...
    fn offset_commit_generation_id_or_member_epoch_matches_java() {
        // Kafka 4.0.0 OffsetCommitRequest.json GenerationIdOrMemberEpoch is
        // versions 1+ (INT32 after GroupId / before MemberId; default -1).
        // Official Java OffsetCommitRequestData.generationIdOrMemberEpoch /
        // OffsetCommitRequest.DEFAULT_GENERATION_ID. Encode already writes
        // generation_id. Decode previously discarded it. Kafka 4.0
        // validVersions is 2-9 (v0–v1 removed). This crate speaks 2–9.
        // This is not SyncGroup GenerationId / Heartbeat GenerationId /
        // JoinGroup response GenerationId / TxnOffsetCommit GenerationId /
        // GroupInstanceId.
        let topics = [OffsetTopic {
            topic: "t".into(),
            partitions: vec![OffsetPartition {
    ...
    fn offset_fetch_request_for_groups_matches_java() {
        // Java 4.0 OffsetFetchRequest.Builder(Map
        // groupIdToTopicPartitionMap, boolean requireStable, boolean
        // throwOnFetchStableOffsetsUnsupported): one group per map
        // entry; Topics from the partition list; null list is all
        // partitions; MemberId null / MemberEpoch -1; RequireStable from
        // the argument. Official Java OffsetFetchRequest.Builder(Map,
        // boolean, boolean). Encode still writes independently. This
        // crate speaks 1-9 (batched Groups need v8+). This is not
        // for_group / from_partitions / Builder.build / groups /
        // getErrorResponse.
