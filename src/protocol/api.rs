...
impl TopicMetadata {
    ...
    /// Java `MetadataResponse.TopicMetadata(Errors, String, boolean, List)`.
    ///
    /// TopicId is zeros (`Uuid.ZERO_UUID`). `authorizedOperations` is
    /// [`MetadataResponse::AUTHORIZED_OPERATIONS_OMITTED`]. Name is the
    /// argument (Java `String` is non-null). The six-argument Java
    /// constructor takes TopicId and authorizedOperations; this helper
    /// is the four-argument overload. Encode still writes independently
    ...
    /// is not [`Self::error`] / [`MetadataResponse::errors`] /
...
impl MetadataRequest {
    ...
    /// Java `MetadataRequest.Builder(List names, boolean allowAuto, short allowedVersion)`.
    ///
...
impl ProducePartitionResponse {
    ...
    /// [`ProduceResponse::to_data`] / [`ProduceRequest::error_response`].
    #[must_use]
    pub fn partition_response_with_record_errors(
        topic: impl Into<String>,
        partition: i32,
        error_code: i16,
        base_offset: i64,
        ...
        let mut part = Self::partition_response_with_offsets(
            topic,
            partition,
            error_code,
            base_offset,
        ...
        part.record_errors = record_errors;
        part
    }
    ...
    #[expect(
        ...
        reason = "Java PartitionResponse six-arg plus topic and partition stored on this type"
    )]
    pub fn partition_response_with_record_errors_and_message(
        topic: impl Into<String>,
        partition: i32,
        error_code: i16,
        base_offset: i64,
        ...
        let mut part = Self::partition_response_with_record_errors(
            topic,
            partition,
            error_code,
            base_offset,
...
/// Encode Produce with NodeEndpoints and an explicit ThrottleTimeMs.
...
mod tests {
    ...
    #[test]
    fn produce_partition_response_with_record_errors_matches_java() {
        // Java 4.0 ProduceResponse.PartitionResponse(Errors, long, long,
        // long, List): baseOffset / logAppendTime / logStartOffset /
        // recordErrors from the arguments; errorMessage null; lastOffset
        // stays INVALID_OFFSET (not a ProduceResponse field). Official
        // Java ProduceResponse.PartitionResponse(Errors, long, long,
        // long, List). This crate speaks 3–12. This is not
        // PartitionResponse(Errors) / PartitionResponse(Errors, String) /
        // PartitionResponse(Errors, long, long, long) / toData /
        // getErrorResponse.
        assert_eq!(
            ProducePartitionResponse::partition_response_with_offsets(
            ...
            ProducePartitionResponse::partition_response_with_record_errors(
                "t",
                0,
                0,
                ProducePartitionResponse::INVALID_OFFSET,
        ...
        let with = ProducePartitionResponse::partition_response_with_record_errors(
            "t",
            1,
        ...
        assert_eq!(with.topic, "t");
        assert_eq!(with.partition, 1);
        assert_eq!(with.error_code, crate::error::INVALID_RECORD);
        assert_eq!(with.base_offset, 7);
        assert_eq!(with.log_append_time_ms, 99);
        assert_eq!(with.log_start_offset, 3);
        assert_eq!(with.current_leader_id, MetadataResponse::NO_LEADER_ID);
        ...
        assert!(with.error_message.is_none());
        leftover_produce_partition_response_with_record_errors(3, std::slice::from_ref(&with));
        leftover_produce_partition_response_with_record_errors(3, &[]);
        leftover_produce_partition_response_with_record_errors(5, std::slice::from_ref(&with));
        leftover_produce_partition_response_with_record_errors(5, &[]);
        leftover_produce_partition_response_with_record_errors(8, std::slice::from_ref(&with));
        leftover_produce_partition_response_with_record_errors(8, &[]);
        leftover_produce_partition_response_with_record_errors(12, std::slice::from_ref(&with));
        leftover_produce_partition_response_with_record_errors(12, &[]);
    }

    fn leftover_produce_partition_response_with_record_errors(
        version: i16,
        parts: &[ProducePartitionResponse],
    ) {
        let mut buf = BytesMut::new();
        ...
        leftover_empty(
            &cur,
            match (version, parts.is_empty()) {
                (3, false) => "Produce v3 PartitionResponse.recordErrors leftover-empty",
                (3, true) => "Produce v3 PartitionResponse.recordErrors empty leftover-empty",
                (5, false) => "Produce v5 PartitionResponse.recordErrors leftover-empty",
                (5, true) => "Produce v5 PartitionResponse.recordErrors empty leftover-empty",
                (8, false) => "Produce v8 PartitionResponse.recordErrors leftover-empty",
                (8, true) => "Produce v8 PartitionResponse.recordErrors empty leftover-empty",
                (12, false) => "Produce v12 PartitionResponse.recordErrors leftover-empty",
                (12, true) => "Produce v12 PartitionResponse.recordErrors empty leftover-empty",
                _ => "Produce PartitionResponse.recordErrors leftover-empty",
            },
        )
        .unwrap();
    }
    ...
    #[test]
    fn produce_partition_response_with_record_errors_and_message_matches_java() {
        // Java 4.0 ProduceResponse.PartitionResponse(Errors, long, long,
        // long, List, String): baseOffset / logAppendTime /
        // logStartOffset / recordErrors / errorMessage from the
        // arguments; lastOffset stays INVALID_OFFSET (not a
        // ProduceResponse field). Official Java
        // ProduceResponse.PartitionResponse(Errors, long, long, long,
        // List, String). This crate speaks 3–12. This is not
        // PartitionResponse(Errors) / PartitionResponse(Errors, String) /
        // PartitionResponse(Errors, long, long, long) /
        // PartitionResponse(Errors, long, long, long, List) / toData /
        // getErrorResponse.
        assert_eq!(
            ProducePartitionResponse::partition_response_with_record_errors(
                "t",
        ...
        let with = ProducePartitionResponse::partition_response_with_record_errors_and_message(
            "t",
            1,
            crate::error::INVALID_RECORD,
            7,
        ...
        assert_eq!(with.error_message.as_deref(), Some("batch dropped"));
        leftover_produce_partition_response_with_record_errors_and_message(
            3,
            std::slice::from_ref(&with),
        );
        leftover_produce_partition_response_with_record_errors_and_message(3, &[]);
        leftover_produce_partition_response_with_record_errors_and_message(
            5,
            std::slice::from_ref(&with),
        );
        leftover_produce_partition_response_with_record_errors_and_message(5, &[]);
        leftover_produce_partition_response_with_record_errors_and_message(
            8,
            std::slice::from_ref(&with),
        );
        leftover_produce_partition_response_with_record_errors_and_message(8, &[]);
        leftover_produce_partition_response_with_record_errors_and_message(
            12,
            std::slice::from_ref(&with),
        );
        leftover_produce_partition_response_with_record_errors_and_message(12, &[]);
    }

    fn leftover_produce_partition_response_with_record_errors_and_message(
        version: i16,
        parts: &[ProducePartitionResponse],
    ) {
        let mut buf = BytesMut::new();
        ...
        leftover_empty(
            &cur,
            match (version, parts.is_empty()) {
                (3, false) => "Produce v3 PartitionResponse.recordErrorsAndMessage leftover-empty",
                (3, true) => {
                    "Produce v3 PartitionResponse.recordErrorsAndMessage empty leftover-empty"
                }
                (5, false) => "Produce v5 PartitionResponse.recordErrorsAndMessage leftover-empty",
                (5, true) => {
                    "Produce v5 PartitionResponse.recordErrorsAndMessage empty leftover-empty"
                }
                (8, false) => "Produce v8 PartitionResponse.recordErrorsAndMessage leftover-empty",
                (8, true) => {
                    "Produce v8 PartitionResponse.recordErrorsAndMessage empty leftover-empty"
                }
                (12, false) => {
                    "Produce v12 PartitionResponse.recordErrorsAndMessage leftover-empty"
                }
                (12, true) => {
    ...
    #[test]
    fn metadata_response_topic_metadata_matches_java() {
        // Java 4.0 MetadataResponse.TopicMetadata(Errors, String, boolean,
        // List): TopicId ZERO_UUID; authorizedOperations
        // AUTHORIZED_OPERATIONS_OMITTED. Official Java
        // MetadataResponse.TopicMetadata(Errors, String, boolean, List).
        // This crate speaks 1–13. This is not getErrorResponse /
        // TopicMetadata(Errors, String, Uuid, boolean, List, int) /
        // errors / toString.
        assert_eq!(
            TopicMetadata::error(0, Some("t"), [0; 16]),
            TopicMetadata::new(0, "t", false, Vec::new())
        );
        let with = TopicMetadata::new(
            crate::error::UNKNOWN_TOPIC_OR_PARTITION,
            "missing",
            true,
            Vec::new(),
        ...
        assert_eq!(with.topic_id, [0u8; 16]);
        assert!(with.is_internal);
        assert!(with.partitions.is_empty());
        assert_eq!(
            with.topic_authorized_operations,
            MetadataResponse::AUTHORIZED_OPERATIONS_OMITTED
        );
        leftover_metadata_topic_metadata(1, std::slice::from_ref(&with));
        leftover_metadata_topic_metadata(1, &[]);
        leftover_metadata_topic_metadata(8, std::slice::from_ref(&with));
        leftover_metadata_topic_metadata(8, &[]);
        leftover_metadata_topic_metadata(10, std::slice::from_ref(&with));
        leftover_metadata_topic_metadata(10, &[]);
        leftover_metadata_topic_metadata(13, std::slice::from_ref(&with));
        leftover_metadata_topic_metadata(13, &[]);
    }

    fn leftover_metadata_topic_metadata(version: i16, topics: &[TopicMetadata]) {
        let resp = MetadataResponse {
            throttle_time_ms: 0,
            brokers: Vec::new(),
            cluster_id: None,
        ...
        leftover_empty(
            &cur,
            match (version, topics.is_empty()) {
                (1, false) => "Metadata v1 TopicMetadata leftover-empty",
                (1, true) => "Metadata v1 TopicMetadata empty leftover-empty",
                (8, false) => "Metadata v8 TopicMetadata leftover-empty",
                (8, true) => "Metadata v8 TopicMetadata empty leftover-empty",
                (10, false) => "Metadata v10 TopicMetadata leftover-empty",
                (10, true) => "Metadata v10 TopicMetadata empty leftover-empty",
    ...
    #[test]
    fn metadata_request_for_topic_names_version_matches_java() {
        // Java 4.0 MetadataRequest.Builder(List names, boolean allowAuto,
        // short allowedVersion): Topics and AllowAutoTopicCreation match
        // Builder(List, boolean); oldest and latest allowed versions are
        // both allowedVersion. Official Java
        ...
        assert_eq!(named_latest, 4);
        assert_eq!(
            MetadataRequest::for_topic_names_version(None, false, 1),
            (None, false, 1, 1)
        );
        let (from_names, from_allow) = MetadataRequest::for_topic_names(Some(&names), false);
        assert_eq!(named, from_names);
        assert_eq!(named_allow, from_allow);
        leftover_metadata_for_topic_names_version(1, None, true, 13);
        leftover_metadata_for_topic_names_version(13, None, true, 13);
        leftover_metadata_for_topic_names_version(4, None, false, 4);
        leftover_metadata_for_topic_names_version(13, None, false, 4);
        leftover_metadata_for_topic_names_version(1, Some(&names), true, 13);
        leftover_metadata_for_topic_names_version(13, Some(&names), true, 13);
        leftover_metadata_for_topic_names_version(4, Some(&names), false, 4);
        leftover_metadata_for_topic_names_version(13, Some(&names), false, 4);
        leftover_metadata_for_topic_names_version(1, Some(&[]), true, 1);
    ...
    fn leftover_metadata_for_topic_names_version(
        version: i16,
        names: Option<&[String]>,
        allow_auto: bool,
        allowed_version: i16,
        ...
        let (topics, allow, oldest, latest) =
            MetadataRequest::for_topic_names_version(names, allow_auto, allowed_version);
        assert_eq!(allow, allow_auto);
        assert_eq!(oldest, allowed_version);
        assert_eq!(latest, allowed_version);
        let mut buf = BytesMut::new();
        encode_metadata_request_topics(&mut buf, version, topics.as_deref(), allow, false).unwrap();
        let mut cur = buf.as_ref();
        let (got, allow_got, include_topic, include_cluster) =
