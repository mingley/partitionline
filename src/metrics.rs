...
pub struct ConsumerMetrics {
    ...
    /// Successful OffsetFetch responses (committed lookups and assignment starts).
    /// KL-07 diagnosis: distinguish OffsetFetch health from fetch/commit traffic.
    pub offset_fetch_ok: u64,
    /// Failed OffsetFetch attempts (transport/protocol errors and broker codes).
    pub offset_fetch_fail: u64,
}
...
mod tests {
    ...
    fn metrics_default_zero() {
        ...
        assert_eq!(ConsumerMetrics::default().offset_fetch_ok, 0);
        assert_eq!(ConsumerMetrics::default().offset_fetch_fail, 0);
