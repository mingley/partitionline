...
async fn main() -> partitionline::Result<()> {
    ...
    if format == "prom" {
        ...
        println!("# HELP partitionline_find_coordinator_ok FindCoordinator successes");
        println!("# TYPE partitionline_find_coordinator_ok counter");
        println!(
            "partitionline_find_coordinator_ok {}",
            cm.find_coordinator_ok
        );
        println!("# HELP partitionline_find_coordinator_fail FindCoordinator failures");
        println!("# TYPE partitionline_find_coordinator_fail counter");
        println!(
            "partitionline_find_coordinator_fail {}",
            cm.find_coordinator_fail
        );
        ...
        println!(
            "fetched {} records rounds={} bytes={} errors={} find_coordinator_ok={} find_coordinator_fail={} fetch_us={} p50_us={} p99_us={} topics={}",
            recs.len(),
            cm.fetch_rounds,
            cm.bytes_fetched,
            cm.fetch_errors,
