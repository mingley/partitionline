...
async fn main() -> partitionline::Result<()> {
    ...
    if format == "prom" {
        ...
        println!("# HELP partitionline_sasl_authenticate_ok SASL authenticate successes");
        println!("# TYPE partitionline_sasl_authenticate_ok counter");
        println!("partitionline_sasl_authenticate_ok {}", cm.sasl_authenticate_ok);
        println!("# HELP partitionline_sasl_authenticate_fail SASL authenticate failures");
        println!("# TYPE partitionline_sasl_authenticate_fail counter");
        println!("partitionline_sasl_authenticate_fail {}", cm.sasl_authenticate_fail);
        ...
        println!(
            "fetched {} records rounds={} bytes={} errors={} sasl_authenticate_ok={} sasl_authenticate_fail={} fetch_us={} p50_us={} p99_us={} topics={}",
            recs.len(),
            cm.fetch_rounds,
            cm.bytes_fetched,
            cm.fetch_errors,
            cm.sasl_authenticate_ok,
            cm.sasl_authenticate_fail,
