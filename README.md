...
groups, transactions, log dirs, quotas, the KRaft metadata quorum
(`describe_metadata_quorum`), and the other Kafka 3.x / 4.x admin APIs
are on `Admin`. Method-level rustdoc names the matching Java
`Admin` / `*Options` calls.
