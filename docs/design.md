...
`Admin::list_topics` / `Admin::describe_topics` / `Admin::describe_topics_with` are Java `listTopics` /
`describeTopics` / `DescribeTopicsOptions.includeAuthorizedOperations`. `Admin::describe_acls_with` is Java
...
- Metadata v1–v8 are classic; v9–v13 are flexible (compact arrays/strings plus tagged fields; request header 2, response header 1). v12 supports topicId. v13 adds top-level ErrorCode INT16 after Topics (and after ClusterAuthorizedOperations on v8–v10) and before tagged fields. `describe_topics_with` is Java `DescribeTopicsOptions.includeAuthorizedOperations` (Metadata v8+ IncludeTopicAuthorizedOperations; TopicAuthorizedOperations on each topic). Kafka 4.0 `validVersions` is `0-13`. This crate speaks 1–13. v0 (empty array means all topics) and v14+ are not spoken.
