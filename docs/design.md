...
`describeTopics` (DescribeTopicPartitions api 75, KIP-966; Metadata fallback when api 75 is missing) / `DescribeTopicsOptions.includeAuthorizedOperations` / `DescribeTopicsOptions.timeoutMs` / `describeTopics(TopicCollection.ofTopicIds)` (Metadata v10+). `Admin::describe_acls_with` is Java
