...
- DescribeTransactions v0 is flexible (compact strings/arrays plus tagged fields; request header 2, response header 1). Kafka 4.0 `validVersions` is `"0"`. This crate speaks 0. v1+ is not spoken. ErrorCode is per transactional.id. `describe_transactions` sends FindCoordinator v4+ CoordinatorKeys of N (`key_type=1`) and one DescribeTransactions RPC per coordinator.
