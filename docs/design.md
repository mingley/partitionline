...
- OffsetCommit v7 is classic (GroupId / GenerationId / MemberId / GroupInstanceId, committed leader epoch, metadata). v8–v9 are flexible (compact strings/arrays plus tagged fields on partitions / topics / top-level; request header 2, response header 1). v9 is KIP-848 error codes (same layout as v8). Kafka 4.0 `validVersions` is `2-9`. This crate speaks 7–9. v2–v6 (retention time) and v10+ are not spoken.
- OffsetFetch v5 is classic (GroupId + Topics, committed leader epoch, top-level error). v6–v9 are flexible (request header 2, response header 1). v7 adds RequireStable (`true` when `isolation.level` is read-committed). v8 replaces GroupId / Topics with Groups (KIP-709; this crate sends one group). v9 adds MemberId / MemberEpoch on each group (KIP-848; classic groups send null / `-1`). Kafka 4.0 `validVersions` is `1-9`. This crate speaks 5–9. v1–v4 (no leader epoch) and v10+ (topic IDs) are not spoken.
- Heartbeat v3 is classic (GroupId, GenerationId, MemberId, GroupInstanceId). v4 is flexible (compact strings plus tagged fields; request header 2, response header 1). Kafka 4.0 `validVersions` is `0-4`. This crate speaks 3–4. v0–v2 (no instance id) and v5+ are not spoken.
- SyncGroup v3 is classic (GroupId, GenerationId, MemberId, GroupInstanceId, Assignments). v4–v5 are flexible (compact strings/bytes/arrays plus tagged fields; request header 2, response header 1). v5 adds ProtocolType / ProtocolName (KIP-559; this crate sends `"consumer"` and the selected assignor). Kafka 4.0 `validVersions` is `0-5`. This crate speaks 3–5. v0–v2 (no instance id) and v6+ are not spoken.
- JoinGroup v5 is classic (GroupInstanceId). v6–v9 are flexible (compact strings/bytes/arrays plus tagged fields; request header 2, response header 1). v7 response adds ProtocolType (KIP-559) and nullable ProtocolName. v8 adds Reason (KIP-800; this crate sends null). v9 adds SkipAssignment; when true the leader does not run the assignor. Kafka 4.0 `validVersions` is `2-9`. This crate speaks 5–9. v2–v4 (no instance id) and v10+ are not spoken.
- LeaveGroup v0–v3 are classic; v4–v5 are flexible (compact strings/arrays plus tagged fields; request header 2, response header 1). v0–v2 are GroupId + MemberId (v1 and v2 match v0). v1+ ThrottleTimeMs. v3 Members + GroupInstanceId. v5 Reason (KIP-800). Kafka 4.0 `validVersions` is `0-5`. This crate speaks 0–5. Classic `ConsumerGroup::leave` negotiates 0–5 (prefers v5; `group.instance.id` on v3+). Admin `removeMembersFromConsumerGroup` stays v3–v5. v6+ is not spoken.
