...
|---|---|---|---|
| Produce (acks, linger, batches, offsets, `delivery.timeout.ms`, `max.block.ms`, `retry.backoff.ms`) | yes (to Metadata leader; retriable errors refresh and retry with exponential `retry_backoff` / `retry_backoff_max`; `delivery_timeout` caps queue-to-ack; `max_block` caps `send` metadata wait) | yes | **done** |
| Fetch with manual assignment | yes (to Metadata leader; retriable errors wait `retry.backoff.ms` then refresh and retry; preferred-replica redirects are immediate; Fetch sends Metadata `leader_epoch`; `ConsumerRecords` including `nextOffsets`) | yes | **done** |
