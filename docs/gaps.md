...
| Admin: CreateTopics, DeleteTopics, DescribeConfigs | yes (CreateTopics v0–7, v5+ flexible, v5 KIP-525 configs, v7 TopicId; DeleteTopics v0–6, v4+ flexible, v5 ErrorMessage, v6 TopicId; DescribeConfigs v0–1); `NewPartitions` for CreatePartitions; admin RPCs wait exponential `retry.backoff.ms` / `retry.backoff.max.ms` on `NOT_CONTROLLER`, coordinator moves, and retriable IO; idle bootstrap sockets close after `connections.max.idle.ms` | yes | **done** |
