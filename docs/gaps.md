...
| Idempotent produce (`enable.idempotence`, PID/epoch/seq) | yes (`InitProducerId` v0–v2, v2 flexible; per-partition sequences, one TCP conn per partition, acks=all, max in-flight 5; `flush` fails on broker error) | yes | **done** |
