...
| Classic consumer groups (join / sync / heartbeat / commit) | yes (range then sticky over all partitions; several topics via `join_topics`; cooperative-sticky / KIP-429; `group.instance.id`; heartbeat loop; rebalance; LeaveGroup; coordinator sockets close after `connections.max.idle.ms`) | yes | **done** |
...
| Transactions / EOS | yes (`transactional.id`, `transaction.timeout.ms` on InitProducerId, `init_transactions`, begin/commit/abort, AddPartitionsToTxn / AddOffsetsToTxn / EndTxn / TxnOffsetCommit) | yes | **done** |
