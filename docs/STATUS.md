...
**Trusted Publishing owner checklist (2026-09-05, tip pending):** `docs/trusted-publishing-owner.md` is the blank crates.io UI binding record (`UNFILLED — not evidence`); linked from RELEASE.md + ADOPTION.md. Workflow shape (`check-trusted-publishing-ready`) stays green separately — UI binding is still owner. Does **not** configure Trusted Publishing. Does **not** lift Suite HOLD.
