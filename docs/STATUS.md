...
**KL-07 broker reconnect metrics + diagnosis cookbook (2026-09-05, tip `4e9eb7d`):** Produce/Fetch accumulate `broker_reconnect_failures` on refused TCP/TLS/SASL connects; guide diagnosis table (reconnect flap / transient outage / blocked consumer); mock tests `tests/reconnect_metrics.rs`. Does **not** close full KL-07 (no two-user exercise; throttle/queue-age/lag recipes may land separately). Does **not** lift Suite HOLD.
