...
**KL-07 roundtrip/intercept external crate-consumers (2026-09-05, tip `ee2dd40`):** `scripts/ci-example-roundtrip-intercept-crate-consumers.sh` cargo-checks `examples/{roundtrip,intercept,consume_intercept}.rs` as downstream bins against the packed `.crate`. Wired into branch-lite / cut-path / publish-ready + bars; guide + migrate checklist. Does **not** close full KL-07 (two-user diagnosis / other example consumers may land separately). Does **not** lift Suite HOLD.
