...
**KL-07 broker throttle metrics + diagnosis cookbook (2026-09-05, tip `5669aaa`):** Produce/Fetch accumulate `broker_throttle_ms_total`; guide diagnosis table (throttle / stale leader / blocked consumer); mock tests `tests/throttle_metrics.rs`. Does **not** close full KL-07 (no two-user exercise; no instrumentation cost measurement). Does **not** lift Suite HOLD.
