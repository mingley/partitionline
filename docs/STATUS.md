...
**KL-07 admin/group/eos external crate-consumers (2026-09-05, tip `PENDING`):** `examples/admin.rs`, `examples/group.rs`, and `examples/eos.rs` cargo-check as downstream bins of a package that depends on the packed `.crate` (`scripts/ci-example-ops-crate-consumers.sh`; wired into branch-lite / cut-path / publish-ready + bars). Does **not** close full KL-07 (produce/consume/txn/metrics consumers and two-user diagnosis may land separately). Does **not** lift Suite HOLD.
