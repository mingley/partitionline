...
**KL-07 optional Prometheus text exporter (2026-09-05, tip `528e935`):** `examples/metrics` + `prom_format.rs` emit `FORMAT=prom` text; `tests/metrics_exporter.rs` proves rendering without a broker and without a prometheus core dependency. Does **not** close full KL-07 (no two-user diagnosis; other metric cookbooks may land separately). Does **not** lift Suite HOLD.
