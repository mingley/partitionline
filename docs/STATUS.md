...
**KL-07 instrumentation cost measured (2026-09-05, tip `PENDING`):** Always-on metrics microbench (`LatencyTracker::record`/`snapshot`, produce counter atomics) + optional `tracing` span enter/exit measured brokerless (`scripts/measure-instrumentation-cost.sh`, `tests/instrumentation_cost.rs`); guide documents catastrophic bounds; ROADMAP 2% produce-ack budget explicitly **not** claimed. Does **not** close full KL-07 (package consumers, diagnosis cookbooks, two independent human runs remain). Does **not** lift Suite HOLD.
