...
**KL-07 queue-age diagnosis Partial (2026-09-05, tip `f8cfbb0`):** `ProducerMetrics::queue_latency` (accept→local-batch drain) distinct from `ack_latency`; guide cookbook + `tests/queue_age.rs`. Does **not** close full KL-07 (throttle/reconnect/lag human runs remain). Does **not** lift Suite HOLD.
