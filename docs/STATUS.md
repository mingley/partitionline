...
**KL-07 produce-retries diagnosis Partial (2026-09-05, tip `PENDING`):** `ProducerMetrics::produce_retries` counts retriable Produce re-queues (distinct from terminal `produce_errors`); guide cookbook + `tests/produce_retries.rs`. Does **not** close full KL-07 (queue-age/throttle/reconnect/lag human runs remain). Does **not** lift Suite HOLD.
