...
**KL-04 controlled-host latency exercise template (2026-09-05, tip `bce6673`):** `docs/controlled-host-latency-exercise.md` is the blank controlled-host qualification record (`UNFILLED — not evidence`); linked from guide + ROADMAP. Shared-runner / local-native gates stay unsigned smoke. Does **not** close full KL-04 (no filled controlled-host cell). Does **not** lift Suite HOLD.
