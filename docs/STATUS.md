...
**KL-07 examples as external crate consumers (2026-09-05, tip ):** `scripts/ci-example-crate-consumers.sh` cargo-checks `examples/{produce,consume,txn}` as bins of a downstream package on the packed `.crate`; wired into branch-lite / cut-path / publish-ready + bars. Does **not** close full KL-07 (no live-broker example run; migrate doc is narrative-only; no two-user diagnosis). Does **not** lift Suite HOLD.
