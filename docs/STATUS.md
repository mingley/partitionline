...
**KL-07 stale-leader diagnosis cookbook (2026-09-05, tip `488f5ad`):** guide Diagnosis cookbook uses tip `produce_errors` / `ack_latency` / `current_lag`; mock `tests/stale_leader_diagnosis.rs` proves permanent `NOT_LEADER_OR_FOLLOWER` bumps `produce_errors` without payload logging. Does **not** close full KL-07 (no two-user exercise; throttle/queue-age/reconnect recipes may land separately). Does **not** lift Suite HOLD.
