...
**KL-07 pause/offsets/wakeup external crate-consumers (2026-09-05, tip `14bcac1`):** `scripts/ci-example-consumer-control-crate-consumers.sh` cargo-checks `examples/{pause,offsets,wakeup}.rs` as downstream bins against the packed `.crate` (tokio `signal` for wakeup). Wired into branch-lite / cut-path / publish-ready + bars; guide. Does **not** close full KL-07 (two-user diagnosis / other example consumers may land separately). Does **not** lift Suite HOLD.
