...
**KL-07 idle/unassigned consumer diagnosis (2026-09-05, tip `6a3c01b`):** Guide cookbook + mock proof that an idle consumer is diagnosed via empty `assignment()` and the Java-shaped fetch error (not payload logging): `tests/idle_unassigned_diagnosis.rs`. Does **not** close full KL-07 (no two-user diagnosis; other cookbooks may land separately). Does **not** lift Suite HOLD.
