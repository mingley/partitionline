...
**KL-07 metrics example external crate-consumer (2026-09-05, tip `751a79c`):** `examples/metrics.rs` cargo-checks as a downstream bin of a package that depends on the packed `.crate` (`scripts/ci-example-metrics-crate-consumer.sh`; wired into branch-lite / cut-path / publish-ready + bars). Does **not** close full KL-07 (produce/consume/txn consumers and two-user diagnosis remain). Does **not** lift Suite HOLD.
