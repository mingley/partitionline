...
**KL-07 offset-commit diagnosis Partial (2026-09-05, tip `14adf15`):** `ConsumerMetrics::offset_commit_ok` / `offset_commit_fail` count OffsetCommit outcomes (sync/async/auto via `commit_with_metadata_timeout`); guide cookbook + `tests/offset_commit_metrics.rs`. Does **not** close full KL-07 (produce-retries/queue-age/throttle/reconnect/lag human runs remain). Does **not** lift Suite HOLD.
