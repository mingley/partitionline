...
**KL-07 lag diagnosis Partial (2026-09-05, tip `27bb5a7`):** guide lag cookbook via `Consumer::current_lag`; mock `tests/lag_diagnosis.rs` proves behind → caught-up without payload logging. Does **not** close full KL-07 (no queue-age/throttle/reconnect recipe close; no two independent human diagnosis runs). Does **not** lift Suite HOLD.
