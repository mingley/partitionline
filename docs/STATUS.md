...
**KL-07 SASL authenticate diagnosis Partial (2026-09-05, tip `PENDING`):** `ConsumerMetrics::sasl_authenticate_ok` / `sasl_authenticate_fail` count terminal SASL authenticate outcomes; guide cookbook + `tests/sasl_authenticate_metrics.rs`. Does **not** close full KL-07. Does **not** lift Suite HOLD.
