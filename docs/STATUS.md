...
**KL-07 share-ack diagnosis Partial (2026-09-05, tip `PENDING`):** `ShareMetrics::acknowledge_errors` counts terminal ShareAcknowledge failures (distinct from `records_acknowledged`); guide cookbook + `tests/share_ack_metrics.rs`. Does **not** close full KL-07 (offset-commit/produce-retries/queue-age/throttle/reconnect/lag human runs remain). Does **not** lift Suite HOLD.
