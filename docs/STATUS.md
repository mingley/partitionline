...
**KL-07 metrics topic-cardinality bound (2026-09-05, tip `3ffc893`):** `MAX_TOPIC_METRIC_SERIES` (256) caps distinct topic rows in producer/consumer/share snapshots; aggregates still count every record; guide documents topic-only labels. Lib + mock tests. Does **not** close full KL-07 (no two-user diagnosis; other cookbooks may land separately). Does **not** lift Suite HOLD.
