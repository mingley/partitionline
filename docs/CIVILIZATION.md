...
|---|---|---|
| WP-0 Public crate identity | **in progress** | 0.1–0.4 done; `cargo package` green; release workflow ready. **0.5 blocked on owner `CARGO_REGISTRY_TOKEN`** (crate name free on crates.io). |
| WP-1 Real-broker CI | **done** | `broker-smoke` matrix 3.9.1 + 4.0.0; mock `test` job unchanged. |
| WP-2 Adversarial trust | **done** | security.md + audit CI + decode OOM guards + fuzz smoke + libFuzzer targets + `fuzz-smoke` CI. |
...
| WP-7 Stewardship | **done** | Issue/PR templates; CONTRIBUTING; tag-publish workflow. |
