...
named holes (DescribeQuorum, Raft voters, DescribeLogDirs v5)
without a deployment need written here.

**Deployment need (ElectLeaders / api 43):** operators restore preferred
replicas after broker failover via Java `Admin.electLeaders` and
`kafka-leader-election.sh`. Keeping that hole forced a second admin
client (Java or C) beside this crate. ElectLeaders is spoken in this
tree (v0–2, controller hop, `NOT_CONTROLLER` retry, unclean on v1+).
...
| WP-6 Adoption gaps | **in progress** | Template + zstd spike + feature matrix + survey [#85](https://github.com/mingley/partitionline/issues/85) + ADOPTION.md + `docs/schema-companion.md` design (crate waits on crates.io). Packed-crate downstream consumer gate in civilization/publish-ready. Git install pin **`v0.1.0-rc.6`** (pin gate allows docs/scripts tip drift; library changes still require a new rc) (KIP-848 join fix + OIDC/mTLS) until crates.io `0.1.0`. **ElectLeaders (43)** closed with a written operator need (preferred/unclean election after failover). |
