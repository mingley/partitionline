...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::offsets_for_times_ok` /
   `offsets_for_times_fail` count terminal timestamp→offset batch outcomes;
   guide cookbook + mock `tests/offsets_for_times_metrics.rs`. ApiVersions /
   ListOffsets / OffsetFetch / Metadata / Leave / FindCoordinator / heartbeat /
   join-sync / SASL / wakeup / queue-age / throttle / reconnect / lag recipes and
