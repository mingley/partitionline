...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::offset_commit_ok` /
   `offset_commit_fail` count OffsetCommit outcomes; guide cookbook + mock
   `tests/offset_commit_metrics.rs`. Produce-retries / queue-age / throttle /
   reconnect / lag recipes and two independent human diagnosis runs remain open.
   Not Done.
