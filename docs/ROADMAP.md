...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::leave_ok` / `leave_fail` count
   terminal LeaveGroup / leave-heartbeat outcomes; guide cookbook + mock
   `tests/leave_group_metrics.rs`. FindCoordinator / heartbeat / join-sync /
