...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::offset_fetch_ok` /
   `offset_fetch_fail` count terminal OffsetFetch outcomes; guide cookbook +
   mock `tests/offset_fetch_metrics.rs`. ListOffsets / Metadata / Leave /
   FindCoordinator / heartbeat / join-sync / queue-age / throttle / reconnect /
   lag recipes and two independent human diagnosis runs remain open. Not Done.
