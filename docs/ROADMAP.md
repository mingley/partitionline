...
is resolved, and disabled/enabled instrumentation costs and redaction are measured.
   **Partial (2026-09-05):** admin/group/eos examples as external package
   consumers of the packed `.crate`
   (`scripts/ci-example-ops-crate-consumers.sh`). Produce/consume/txn and
   metrics consumers may land separately; two-user diagnosis remains open.
   Not Done.
