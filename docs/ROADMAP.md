...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::sasl_authenticate_ok` /
   `sasl_authenticate_fail` count terminal SASL authenticate outcomes; guide
   cookbook + mock `tests/sasl_authenticate_metrics.rs`. OffsetFetch /
   ListOffsets / Metadata / Leave / FindCoordinator / heartbeat / join-sync /
   queue-age / throttle / reconnect / lag recipes and two independent human
   diagnosis runs remain open. Not Done.
