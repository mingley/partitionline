...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::metadata_refresh_ok` /
   `metadata_refresh_fail` count terminal Metadata refreshes; guide cookbook +
   mock `tests/metadata_refresh_metrics.rs`. Leave / FindCoordinator / heartbeat /
   join-sync / queue-age / throttle / reconnect / lag recipes and two independent
   human diagnosis runs remain open. Not Done.
