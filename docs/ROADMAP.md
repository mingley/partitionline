...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::{rebalances,partitions_revoked,partitions_assigned}`
   + guide rebalance-thrash cookbook + `tests/rebalance_metrics.rs`. Queue-age /
