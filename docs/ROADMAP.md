...
is resolved, and disabled/enabled instrumentation costs and redaction are measured.
   **Partial (2026-09-05):** roundtrip/intercept/consume_intercept examples as external
   package consumers of the packed `.crate`
   (`scripts/ci-example-roundtrip-intercept-crate-consumers.sh`). Produce/consume/txn,
   ops, metrics, and auth consumers may land separately; two-user diagnosis
   remains open. Not Done.
