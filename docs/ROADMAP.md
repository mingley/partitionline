...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ProducerMetrics::{transactions_committed,transactions_aborted}`
   + guide transaction-outcome cookbook + `tests/txn_outcome_metrics.rs`. Queue-age /
   throttle / reconnect / lag / rebalance recipes and two independent human diagnosis
   runs remain open. Not Done.
