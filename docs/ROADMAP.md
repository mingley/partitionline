...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::join_ok` / `join_fail` /
   `sync_ok` / `sync_fail` count classic JoinGroup/SyncGroup outcomes; guide
   cookbook + mock `tests/join_sync_metrics.rs`. Heartbeat / queue-age /
   throttle / reconnect / lag recipes and two independent human diagnosis runs
