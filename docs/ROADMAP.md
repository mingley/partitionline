...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ShareMetrics::acknowledge_errors` counts terminal
   ShareAcknowledge failures (distinct from `records_acknowledged`); guide
   cookbook + mock `tests/share_ack_metrics.rs`. Offset-commit / produce-retries /
