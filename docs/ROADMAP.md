...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `ConsumerMetrics::api_versions_ok` /
   `api_versions_fail` count terminal ApiVersions outcomes; guide
   cookbook + mock `tests/api_versions_metrics.rs`. OffsetFetch /
   ListOffsets / Metadata / Leave / FindCoordinator / heartbeat / join-sync /
   SASL authenticate / queue-age / throttle / reconnect / lag recipes and two
   independent human diagnosis runs remain open. Not Done.
