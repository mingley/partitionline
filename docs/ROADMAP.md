...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** `broker_throttle_ms_total` on produce/fetch metrics
   accumulates broker `ThrottleTimeMs`; guide diagnosis cookbook +
   `tests/throttle_metrics.rs`. Queue age, reconnect, lag recipes and two-user
