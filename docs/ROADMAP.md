...
is resolved, and disabled/enabled instrumentation costs and redaction are measured.
   **Partial (2026-09-05):** pause/offsets/wakeup examples as external
   package consumers of the packed `.crate`
   (`scripts/ci-example-consumer-control-crate-consumers.sh`). Other example
   consumers may land separately; two-user diagnosis remains open. Not Done.
