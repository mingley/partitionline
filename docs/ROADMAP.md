...
2. Extend existing metrics and optional tracing only where diagnosis is missing:
   ...
   **Partial (2026-09-05):** guide stale-leader diagnosis cookbook via tip
   `produce_errors` / `ack_latency` plus mock `tests/stale_leader_diagnosis.rs`
   (permanent `NOT_LEADER_OR_FOLLOWER` → errors rise without payload logging).
   Queue-age / throttle / reconnect / two independent human diagnosis runs
   remain open. Not Done.
