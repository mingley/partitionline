...
echo "       bash scripts/check-trusted-publishing-ready.sh"
echo "     (crates.io → Settings → Trusted Publishing → GitHub: mingley/partitionline,"
echo "      workflow release.yml). Keep Actions secret CARGO_REGISTRY_TOKEN until an"
echo "      OIDC publish succeeds, then remove the long-lived secret."
