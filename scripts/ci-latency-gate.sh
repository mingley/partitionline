...
limit="$(python3 -c "base=int('$base_p99'); slack=int('$SLACK_PCT'); print(base + (base * slack // 100))")"

echo "ci-latency-gate: measured_p99_us=$p99 baseline_p99_us=$base_p99 limit_us=$limit (slack=${SLACK_PCT}%)"
if (( p99 > limit )); then
